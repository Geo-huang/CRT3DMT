...
void GaussTri::init_tri(const unsigned int p)
{
  // fullfill _weights and _points
  assert(p<21);
  this->expand_sub_to_full_order(p);
...
void GaussTri::get_suborder(const unsigned int p,
			     std::vector<unsigned int>& suborder)
{
  unsigned int sub_order_nums[20]=
    {1,1,2,2,3,3,4,5,6,6,7,8,10,10,11,13,15,17,17,19};
  unsigned int sub_order_num= sub_order_nums[p-1]; // 0,19
  suborder.clear(); suborder.resize(sub_order_num);
  // do switch
  switch (p)
    {
    case 1:
...
void GaussTri::get_suborder_L0L1L2_w(const unsigned int p,
				     std::vector<Real>& w,
				     std::vector< std::vector<Real> >& L0L1L2)
{
  unsigned int sub_order_nums[20]=
    {1,1,2,2,3,3,4,5,6,6,7,8,10,10,11,13,15,17,17,19};
  unsigned int sub_order_num= sub_order_nums[p-1]; // 0,19
  w.clear(); w.resize(sub_order_num);
  L0L1L2.clear(); L0L1L2.resize(sub_order_num);
  for(unsigned int i=0; i<sub_order_num; i++)
    { 
      L0L1L2[i].clear();
      L0L1L2[i].resize(3);
    }
  ...
  switch(p)
    ...
    case 1:
      {
	double suborder_xy_rule[1][3] = {
	  {0.333333333333333,  0.333333333333333, 0.333333333333333},
	};
	double suborder_w_rule[1] = {
	  1.000000000000000
	};
	// copy
	for(unsigned int i=0; i<sub_order_num; i++) {
	  w[i]=suborder_w_rule[i];
	  for(unsigned int j=0; j<3; j++)
	    L0L1L2[i][j]=suborder_xy_rule[i][j];
	}
	return ;	
      }
    case 2:
    ...
    case 5:
      {
	double suborder_xy_rule[3][3] = {
	  {0.333333333333333, 0.333333333333333, 0.333333333333333},
	  {0.059715871789770, 0.470142064105115, 0.470142064105115},
	  {0.797426985353087, 0.101286507323456, 0.101286507323456}  
	};
	double suborder_w_rule[3] = {
	  0.225000000000000,
	  0.132394152788506,
	  0.125939180544827
	};
...
void GaussTri::expand_sub_to_full_order(const unsigned int p)
{
  // 1st, get the suborder
  std::vector<unsigned int> suborder;
  this->get_suborder(p,suborder);
  assert(suborder.size()>0 && suborder.size()<20); //[1,19]
  // print suborder
  // for(unsigned int i=0; i<suborder.size(); i++)
  //  std::cout<<suborder[i]<<"\t";
  //  std::cout<<std::endl;
  
  // 2nd, get L(L0L1L2) and w at suborder
  std::vector<Real> w;
  std::vector< std::vector<Real> > L;
  this->get_suborder_L0L1L2_w(p, w, L);
  assert(L.size()==suborder.size());
  for(unsigned int i=0; i<L.size(); i++)
    assert(L[i].size()==3);

  // 3nd, expand to get weights and guass points for "p" on triangle
  // get the total number of gauss points and weights.
  unsigned int n=0;
  for(unsigned int i=0; i<suborder.size(); i++)
    n += suborder[i];
  // init _weights and _points
  this->_points.clear(); 
  this->_weights.clear();
  for(unsigned int i=0; i<suborder.size(); i++)
    {
      if(suborder[i]==1) 
	{
	  // case A A A
	  const Real A=L[i][0];
	  _points.push_back(Point(A,A,A));
	  _weights.push_back(w[i]);
	}
      else if(suborder[i]==3)
	{
	  // case A B B,
	  //      B A B,
	  //      B B A
	  const Real A=L[i][0]; const Real B=L[i][1];
	  _points.push_back(Point(A,B,B));
	  _weights.push_back(w[i]);
	  _points.push_back(Point(B,A,B));
	  _weights.push_back(w[i]);
	  _points.push_back(Point(B,B,A));
	  _weights.push_back(w[i]);	
	}
      else if(suborder[i]==6)
	{
	  // case A B C, A C B,
	  //      B A C, C A B,
	  //      B C A, C B A.
	  const Real A=L[i][0]; const Real B=L[i][1]; const Real C=L[i][2];
	  _points.push_back(Point(A,B,C));
	  _weights.push_back(w[i]);
	  _points.push_back(Point(A,C,B));
	  _weights.push_back(w[i]);
	  _points.push_back(Point(B,A,C));
	  _weights.push_back(w[i]);
	  _points.push_back(Point(C,A,B));
	  _weights.push_back(w[i]);
	  _points.push_back(Point(B,C,A));
	  _weights.push_back(w[i]);
	  _points.push_back(Point(C,B,A));
	  _weights.push_back(w[i]);
	}
      else 
	{
  ...
  // checking....
  assert(_points.size()==_weights.size() && _weights.size()==n);  
